from session_5.window_5 import Ui_MainWindow
        ...
        for i in range(self.tableWidget.rowCount()):
            try:
                prod_id = self.cur.execute("""select id_products from products where title_products = '{}' """.format(
                    self.tableWidget.cellWidget(i, 0).currentText())).fetchone()[0]
                self.count = float(self.tableWidget.item(i, 2).text())
                self.cur.execute("""insert into purchases (id_product, amount, total, price) values (?, ?, ?, ?)""",
                                 (prod_id, self.count, self.count * self.price, self.price))
                self.cur.execute("""update products set count = count + {} where id_products = {}""".format(self.count, prod_id))
                self.con.commit()
            except Exception as ex:
                print(ex)
        try:
